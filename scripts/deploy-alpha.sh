#!/usr/bin/env bash
# =============================================================================
# WaddlePerf Alpha Deployment Script
...
#   NAMESPACE             Target namespace (default: waddleperf-alpha)
#   APP_HOST              Application hostname (default: waddleperf.localhost.local)
#
# =============================================================================
...
readonly APP_NAME="${APP_NAME:-waddleperf}"
readonly KUBE_CONTEXT="${KUBE_CONTEXT:-local-alpha}"
readonly NAMESPACE="${NAMESPACE:-waddleperf-alpha}"
readonly APP_HOST="${APP_HOST:-waddleperf.localhost.local}"
readonly OVERLAY_PATH="${OVERLAY_PATH:-k8s/kustomize/overlays/alpha}"

# Services with Dockerfiles (webui and testserver referenced in kustomization but
# have no Dockerfiles — only unified-api has a Dockerfile)
declare -a SERVICES=("unified-api")
...
OPTIONS:
    ...
    --service SERVICE     Build specific service only (e.g., unified-api)
...
EXAMPLES:
    ...
    $(basename "$0") --service unified-api
